    ...
    Explainers.count((err, count) => {
      if (count === 0) {
        Explainers.create([
          ...
          .then((explainers) => console.log(explainers))
          .then(() => {
            return Explainers.find({});
          })
          .then((explainers) => {
            console.log("explainers from db", explainers);
          })
          .catch((err) => console.log(err));
      }
    });
